...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void updateLockSummary() {
        ...
        } else if (value.contains("hyperion_lock")) {
            resId = R.string.lockscreen_lock_icon_hyperion;
    ...
    private void resizeHyperionLock() {
        Bitmap hyperionLock = BitmapFactory.decodeResource(getResources(), R.drawable.hyperion_lock);
        if (hyperionLock != null) {
            String path = null;
            int px = requestImageSize();
            hyperionLock = Bitmap.createScaledBitmap(hyperionLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                            + "hyperion_lock" + System.currentTimeMillis() + ".png");
                path = image.getAbsolutePath();
                mLockImage.renameTo(image);
                FileOutputStream outPut = new FileOutputStream(image);
                hyperionLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
